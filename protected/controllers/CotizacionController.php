...
class CotizacionController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('index','view','create','update','admin','delete','cotporreq',
					'aceptar','imprimir','cancelarcotizacion'),
				'users'=>array('@'),
			),
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Cotizacion']) && isset($_POST['detalle']))
		{
			try
			{
				...
				$comandoSeguimiento = $conection->createCommand("UPDATE Seguimiento SET estatus_did = 3 WHERE requisicion_aid = " .$requisicion->id)->execute();
				// La Cotización
				...
				if($comandoCotizacion->execute())
				{
					...
					if($comandoRequisicion->execute() || $requisicion->estatus_did == 3)
					{
						$comandoProveedoresPorRequisicion = $conection->createCommand("UPDATE ProveedoresPorRequisicion
							SET estatus_did = 3, fechaCotizacion_ft = :fechaCotizacion WHERE proveedor_aid = :proveedor_aid && requisicion_aid = :requisicion_aid");
						...
						$comandoProveedoresPorRequisicion->bindValue(":fechaCotizacion",date("Y-m-d H:i:s"),PDO::PARAM_STR);
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['detalle']))
		{
			...
			if($model->save()){
				...
				$cotizacion = Cotizacion::model()->findByPK($_GET["id"]);
				...
				foreach($detalleCotizacion as $detalle)
				{
					...
					$sql = "UPDATE DetalleCotizacion SET
							precioUnitario = :precioUnitario,
							importe = :importe
							WHERE articulo_aid = :articulo_aid &&
							cotizacion_did = :cotizacion_did";
					$comandoDetalle = $conection->createCommand($sql);
	...
	{
		$cotizaciones = Cotizacion::model()->findAll(array('order'=>'total ASC', 'condition'=>'estatus_did != 6 && requisicion_did=:r', 'params'=>array(':r'=>$id)));
		...
		try
		{
			...
			if($_GET['tipoCambio']==101)
			{
				/*
					...
					2.- Si estoy Aceptando
						...
						2.6.- Inserto en la tabla de Seguimiento
							2.6.1.- Actualizo el campo fechaCotización, usuarioCotizacion, ,fechaOrdenCompra, usuarioOrdenCompra y el estatus a 4
				*/
				$cotizacion = Cotizacion::model()->find('id = :c', array(':c'=>$id));
				...
				//'OC'+año+00001
				$ultimoAnoRegistrado = $conection->createCommand("
																	SELECT DATE_FORMAT(fecha_f,'%Y') AS fecha
																	FROM OrdenCompra
																	WHERE id = (SELECT MAX(id) FROM OrdenCompra)
																")->queryAll();

				/*
					En caso de que haya sucedido un cambio de año el numero de orden de compra consecutivo se reinicia a 0
					de lo contrario se realiza la consulta y se sigue incrementando
				*/
				if(isset($ultimoAnoRegistrado) && empty($ultimoAnoRegistrado)){

					$numeroCompra = "OC".date('Y').str_pad(1,5,"0",STR_PAD_LEFT);
				}else{
					$ordenCompraSig = 0;
					if($ultimoAnoRegistrado[0]["fecha"] == date('Y')){
						$ordenCompraSig = $conection->createCommand("
																		SELECT
																			CAST(SUBSTRING(numeroOrdenCompra,7,5) AS UNSIGNED) AS numOrdenCompra
																		FROM OrdenCompra
																		WHERE id = (SELECT MAX(id) FROM OrdenCompra);
																	")->queryAll();
					}

					$ordenCompraSig = $ordenCompraSig[0]["numOrdenCompra"];
					$ordenCompraSig++;
					$numeroCompra = "OC".date('Y').str_pad($ordenCompraSig,5,"0",STR_PAD_LEFT);
				}



				$comandoOrden->bindValue(":numeroOrdenCompra",$numeroCompra,PDO::PARAM_STR);
				...
				/*
					1.- Saber la cotización
					2.- Saber la ordencompra


				*/
				date_default_timezone_set('America/Mazatlan');
				$fechaOrdenCompra = date('Y-m-d h:i:s a', time());
				$comandoSeguimiento = $conection->createCommand("UPDATE Seguimiento SET cotizacion_aid = :cotizacion, ordenCompra_aid = :ordenCompra, fechaCotizacion_f = :fechaCotizacion, fechaOrdenCompra_f = :fechaOrdenCompra, usuarioCotizacion = :usuarioCotizacion, usuarioOrdenCompra = :usuarioOrdenCompra, estatus_did = 4 WHERE requisicion_aid = " . $cotizacion->requisicion_did);

				$comandoSeguimiento->bindValue(":cotizacion", $cotizacion->id, PDO::PARAM_INT);
				$comandoSeguimiento->bindValue(":ordenCompra", $ordenCompra->id, PDO::PARAM_INT);
				$comandoSeguimiento->bindValue(":fechaCotizacion", $cotizacion->fechaCreacion_f, PDO::PARAM_STR);
				$comandoSeguimiento->bindValue(":fechaOrdenCompra", $fechaOrdenCompra, PDO::PARAM_STR);
				$comandoSeguimiento->bindValue(":usuarioCotizacion", $cotizacion->proveedor->usuario->usuario, PDO::PARAM_STR);
				$comandoSeguimiento->bindValue(":usuarioOrdenCompra", Yii::app()->user->name, PDO::PARAM_STR);
				...
				//Vamos a ver la impresión de la orden de compra cuando se acepte
				$this->redirect(array('ordenCompra/imprimir','id' => $ordenCompra->id));
			}
			else if($_GET['tipoCambio'] == 2)
			{
				...
				*/
				...
				//Borro el detalle de la orden de compra (no lo borro nada)
				/*
				...
				$comandoDetalle->execute();
				*/
				//Borro la orden de compra (Actualizo mejor a cancelada)
				$ordenCompra = OrdenCompra::model()->find("requisicion_did = " . $_GET['reqid']); //Idenfico la orden de compra
				$comandoOrden = $conection->createCommand("UPDATE OrdenCompra set estatus_did = 4 WHERE id = :orden_id");
				$comandoOrden->bindValue(":orden_id",$ordenCompra->id,PDO::PARAM_STR);
				$comandoOrden->execute();
	...
	public function actionImprimir($id)
	{
		...
		Yii::app()->db->createCommand("insert into Actividad (mensaje, usuario) Values ('Ha entrado a la vista de impresión de la cotización: " .
												$detalleCotizacion[0]->cotizacion->numeroCotizacion . "', '"  .
	...
	public function actionCancelarcotizacion($id)
	{
		$modelCotizacion = Cotizacion::model()->find("id = " . $id);
		$modelCotizacion->estatus_did = 6;
		$criteria=new CDbCriteria;
		$criteria->select='max(id) AS id';
		$criteria->condition = "requisicion_aid = " . $modelCotizacion->requisicion_did . " && proveedor_aid = " . $modelCotizacion->proveedor_aid;
		$idProvPorReq = ProveedoresPorRequisicion::model()->find($criteria);
		$model = ProveedoresPorRequisicion::model()->find("id = " . $idProvPorReq->id);
		/* echo "<pre>"; print_r($model->attributes); echo "</pre>";
		exit;*/
	    $model->estatus_did = 6;

		if($model->save() && $modelCotizacion->save()){
			Yii::app()->user->setFlash('success', '<strong><i class="icon-ok"></i> La cotización se ha Cancelado!</strong>');
			$this->redirect(array('requisicion/index'));
		}
	}
}
