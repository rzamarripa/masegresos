...
class UsuarioController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Usuario']))
		{
            ...
            if($model->validate())
            {
                ...
                $model->comprobarPass = md5($_POST['Usuario']['comprobarPass']);
	...
	public function actionAutocompletesearch()
	{
	    ...
	    if (!empty($q))
	    {
			...
			$criteria->select=array('id', "CONCAT_WS(' ',usuario) as usuario");
			$criteria->condition="lower(CONCAT_WS(' ',usuario)) like lower(:usuario) ";
			$criteria->params=array(':usuario'=>$q);
			$criteria->limit='10';
	       	$cursor = Usuario::model()->findAll($criteria);
			foreach ($cursor as $valor)	
				$result[]=Array('label' => $valor->usuario,  
				                'value' => $valor->usuario,
				                'id' => $valor->id, );
	    }
