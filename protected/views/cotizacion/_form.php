 <div classs="cotizacion" ng-controller="CotizacionController">
			...
			<div class="controls">
				...
				<span class="add-on">Fecha</span>	
								...
								if ($model->fechaEntrega_f!='') 
									$model->fechaEntrega_f=date('Y-m-d',strtotime($model->fechaEntrega_f));
	...
	<div class="span6 ">
		<h4>Observaciones de la Requisición</h4>
		<p><strong><?php print_r($requisicion["comentarios"]); ?></strong></p>
	</div>
				...
				<tr ng-repeat="(key,item) in items">
					...
					<td>
						...
						<span class="control-group" ng-class="{error: item.error.cantidad}" >
							<input value="{{item.cantidad}}" ng-change="calcularTotal(item)" class="input-mini" type="hidden" ng-model="item.cantidad" name="detalle[{{key}}][cantidad]" />
						</span>
					</td>
					<td>
						<div class="input-prepend control-group" ng-class="{error: item.error.precio}">
							<span class="add-on">$</span>
							<input ng-change="calcularTotal(item)" autocomplete="off" class="input-small" type="text" ng-model="item.precio" placeholder="0.00" name="detalle[{{key}}][precioUnitario]" />
						</div>
					</td>
					<td>
